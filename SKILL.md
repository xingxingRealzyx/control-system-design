...
```mermaid
flowchart TD
    S0["0 需求澄清"] --> S1["1 物理建模<br/>图示+推导链+方程"]
...
- **固定产出四件套，缺一不可**：
  1. **物理世界图示**：用 matplotlib 绘制对象示意图并落盘 PNG，必须画出：几何形体（按真实拓扑）、广义坐标及其正方向、输入作用位置与方向、参数标注（M/m/l…）、重力方向与角度参考线。绘制规范与代码骨架见 modeling.md §10。**生成后必须读图目检**，确认与文字描述一致。禁止用文字描述或 mermaid 代替（mermaid 表达不了几何形体）。
  2. **符号定义表**：符号 / 含义 / 取值 / 单位 / 来源（给定|假设）。
  3. **推导链**：从物理世界到方程逐级写出，**禁止从对象描述直接跳到 A 矩阵**——质心位置与速度表达式 → 动能 T / 势能 V → 代入拉格朗日方程 → 非线性运动方程 M(q)q̈ + C(q,q̇)q̇ + G(q) = Bu，每一步的公式都显式展示。
  4. **非线性项处理说明**：摩擦、死区等在线性模型中丢弃，但写入非线性仿真模型。
...
- **线性化过程必须展示**：雅可比逐元素推导（或消元后 q̈ 的显式表达式及 D 的定义），不允许只给结论矩阵。
- A = ∂f/∂x|₍x₀,u₀₎，B = ∂f/∂u|₍x₀,u₀₎，得偏差量 δẋ = Aδx + Bδu 后去掉 δ 记号，给出**代入参数数值的 (A, B, C, D) 完整矩阵**。
...
控制目标 / 性能指标 / 约束 / 输出语言
## 1. 物理建模
简化假设 → 物理世界图示(图片，落盘并引用) → 符号表(含来源标注) → 推导链(质心/能量→拉格朗日方程→非线性方程)
## 2. 状态空间模型
状态选择 → 平衡点 → 线性化(展示代数过程) → (A,B,C,D) 数值矩阵
## 3. s域分析
G(s) → 极零点 → 稳定性 → 时/频域指标
## 4. 能控性与能观性
秩检验过程 → 结论与影响
...
- **面向 GitHub 渲染的 md 文档（README/报告）必须过渲染管线规则**：①块级公式 `$$` 独占一行；②矩阵换行写 `\\\\`（管线会吞一层转义，MathJax 才能收到 `\\`）；③行内公式**开头**的 `$` 前不得紧贴全角标点（，。（：等）——补 ASCII 空格或改写句子，**收尾** `$` 后跟全角标点无碍；④行内公式内不用 `\,` 等反斜杠+标点命令（同样被吞）。发布前用 `gh api --method POST /markdown -f text=... -f mode=gfm` 预检，确认每个公式都产生 math-renderer 存根且无残留原始 `$`。
...
| 文件 | 内容 | 何时读 |
|---|---|---|
| `references/modeling.md` | 建模方法、拉格朗日五步法、线性化、物理图示绘制规范、3 个完整算例 | Stage 1–2 |
